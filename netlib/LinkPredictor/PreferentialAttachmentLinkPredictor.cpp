...
double PreferentialAttachmentLinkPredictor::generateScore( unsigned int vertex, unsigned int neighbor ) {
	unsigned int vertexNeighbors = this->network.outNeighbors( vertex ).size();
	unsigned int neighborNeighbors = this->network.inNeighbors( neighbor ).size();

	return (double)(vertexNeighbors*neighborNeighbors);
}
