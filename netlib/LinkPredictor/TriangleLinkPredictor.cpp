...
#include <cmath>
#include <iostream>
#include "TriangleLinkPredictor.h"

using std::cerr;

TriangleLinkPredictor::TriangleLinkPredictor( const WeightedNetwork& network, const WeightedNetwork& completeNetwork,int selection = -1 ) : LinkPredictor(network,completeNetwork), selection( selection ) {
	if( this->selection > -1 ) {
		return;
	}
...
void TriangleLinkPredictor::computeDetailedNeighbors( const neighbor_set_t& inNeighbors, const neighbor_set_t& outNeighbors, DetailedNeighbors& result ) const {
...
void TriangleLinkPredictor::edgeProfile( vertex_t v1, vertex_t v2, unsigned long vCounts[] ) {
...
unsigned long TriangleLinkPredictor::intersectionCount( const vector<vertex_t>& v1, const vector<vertex_t>& v2 ) const {
...
unsigned long TriangleLinkPredictor::differenceCount( const vector<vertex_t>& v1, const vector<vertex_t>& v2 ) const {
	vector<vertex_t>::const_iterator first1 = v1.begin();
	vector<vertex_t>::const_iterator last1 = v1.end();
	vector<vertex_t>::const_iterator first2 = v2.begin();
	vector<vertex_t>::const_iterator last2 = v2.end();
...
unsigned long TriangleLinkPredictor::unionCount( const vector<vertex_t>& v1, const vector<vertex_t>& v2 ) const {
	vector<vertex_t>::const_iterator first1 = v1.begin();
	vector<vertex_t>::const_iterator last1 = v1.end();
	vector<vertex_t>::const_iterator first2 = v2.begin();
	vector<vertex_t>::const_iterator last2 = v2.end();
...
TriangleLinkPredictor::~TriangleLinkPredictor() {
}

double TriangleLinkPredictor::generateScore( vertex_t vertex, vertex_t neighbor ) {
	unsigned long vCounts[graphlets] = {0};
	this->edgeProfile( vertex, neighbor, vCounts );
